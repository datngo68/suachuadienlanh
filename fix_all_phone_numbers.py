...
def fix_phone_numbers(file_path):
    ...
    try:
        ...
        content = content.replace('033.6288.248', '033.6288.888')
        content = content.replace('0336288248', '0336288888')
        
        # Thêm data attributes cho các link tel: chưa có
        content = re.sub(
            r'<a href="tel:0336288888"([^>]*)>([^<]*)</a>',
            lambda m: f'<a href="tel:0336288888"{m.group(1)} data-phone>{m.group(2)}</a>' if 'data-phone' not in m.group(1) else m.group(0),
            content
        )
        
        # Thêm data attributes cho các link zalo chưa có
        content = re.sub(
            r'<a href="https://zalo\.me/0336288888"([^>]*)>([^<]*)</a>',
            lambda m: f'<a href="https://zalo.me/0336288888"{m.group(1)} data-zalo-link>{m.group(2)}</a>' if 'data-zalo-link' not in m.group(1) else m.group(0),
            content
        )
        ...
        content = re.sub(
            ...
            '<strong data-phone>033.6288.888</strong>',
            content
        )
        
        content = re.sub(
            r'<p><i class="fas fa-phone"></i> 033\.6288\.888</p>',
            '<p><i class="fas fa-phone"></i> <span data-phone>033.6288.888</span></p>',
            content
        )
...
def main():
    ...
    print("1. Sua tat ca so dien thoai tu 033.6288.248 thanh 033.6288.888")
